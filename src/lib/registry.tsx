...
export const { registry } = defineRegistry(catalog, {
  components: {
    ...
    Container: ({ props, children }) => {
      if (props.layout === "grid") {
        ...
        return (
          <div
            ...
            style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gap: `${gapPx}px`, width: "100%" }}
          >
            {children}
          </div>
        );
      ...
      return (
        <div style={{
          display: "flex", flexDirection: isH ? "row" : "column", flexWrap: isH ? "wrap" : undefined,
          gap: `${gapPx}px`, alignItems: "stretch", width: "100%", height: isH ? undefined : "100%",
          minWidth: 0, gridColumn: isH ? "1 / -1" : undefined,
        }}>
          {children}
        </div>
      );
    ...
    StrategyHeader: ({ props }) => {
      const exchangeCls: Record<string, string> = { robinpump: "bg-emerald-500/10 text-emerald-400" };
      const statusCls: Record<string, string> = {
        draft: "bg-secondary text-muted-foreground", active: "bg-emerald-500/10 text-emerald-400",
        paused: "bg-amber-500/10 text-amber-400", error: "bg-red-500/10 text-red-400",
      };
      return (
        <div style={{ gridColumn: "1 / -1" }}>
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.12em] mb-2 block">Strategy</span>
          <h2 className="font-display text-[1.5rem] leading-tight text-foreground mb-1">{props.name}</h2>
          {props.description && <p className="text-sm text-text-secondary leading-relaxed mb-2">{props.description}</p>}
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-bold px-2 py-[3px] rounded-md uppercase tracking-wider ${exchangeCls[props.exchange] ?? "bg-secondary text-muted-foreground"}`}>
              {props.exchange}
            </span>
            <span className={`text-[10px] font-bold px-2 py-[3px] rounded-md capitalize ${statusCls[props.status ?? "draft"]}`}>
              {props.status ?? "draft"}
            </span>
          </div>
        </div>
    ...
    FlowBlock: ({ props }) => {
      ...
      const COLORS: Record<string, { border: string; bg: string; tag: string }> = {
        ...
        violet: { border: "border-violet-500/20", bg: "bg-violet-500/[0.04]", tag: "text-violet-400" },
        emerald: { border: "border-emerald-500/20", bg: "bg-emerald-500/[0.04]", tag: "text-emerald-400" },
      ...
      const ICONS: Record<string, ReactNode> = {
        clock: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 18 18"><circle cx="9" cy="9" r="7" /><path d="M9 5v4l2.5 2.5" /></svg>,
        chart: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 18 18"><path d="M2 13l5-8 4 5 5-7" /></svg>,
        bolt: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 18 18"><path d="M9 2v14M4 7l5-5 5 5" /></svg>,
        coins: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 18 18"><circle cx="7" cy="7" r="5" /><path d="M11.5 7.5a5 5 0 11-4-4" /></svg>,
        alert: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 18 18"><path d="M9 2L2 15h14L9 2z" /><path d="M9 7v3M9 13h.01" /></svg>,
        repeat: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 18 18"><path d="M3 9a6 6 0 0112 0M15 9a6 6 0 01-12 0" /><path d="M13 7l2 2 2-2M5 11l-2-2-2 2" /></svg>,
      };
      const c = COLORS[props.color] ?? COLORS.blue!;
      const icon = ICONS[props.icon ?? "bolt"] ?? ICONS.bolt;
      return (
        <div className={`flex-1 min-w-[140px] border ${c.border} ${c.bg} rounded-xl p-3 flex items-center`}>
          <div className="flex items-center gap-2.5 w-full">
            <div className={`shrink-0 ${c.tag}`}>{icon}</div>
            <div className="min-w-0">
              <span className={`text-[10px] font-bold uppercase tracking-[0.12em] ${c.tag}`}>{props.tag}</span>
              <p className="text-sm font-medium text-foreground mt-0.5 wrap-break-word">{
                props.label.replace(/\{(\w+)\}/g, (_m, key: string) => {
                  ...
                  if (typeof val === "string" && val.startsWith("0x")) return val.length > 14 ? `${val.slice(0, 6)}…${val.slice(-4)}` : val;
    ...
    Connector: ({ props }) => (
      <div className="hidden sm:flex items-center justify-center shrink-0 px-0.5">
        {props.style === "arrow" ? (
          ...
          <div className={`h-px w-4 ${props.style === "dashed" ? "border-t border-dashed border-border-light/60" : "bg-border-light/60"}`} />
        )}
      </div>
    ),
    ...
    MetricCard: ({ props }) => {
      const { get } = useStateStore();
      const trendColor = props.trend === "up" ? "text-emerald-400" : props.trend === "down" ? "text-red-400" : "text-muted-foreground";
      ...
      if (resolved == null) {
        ...
        isAddress = true;
        displayValue = resolved.length > 14 ? `${resolved.slice(0, 6)}…${resolved.slice(-4)}` : resolved;
      } else if (typeof resolved === "number" || (typeof resolved === "string" && resolved !== "" && !isNaN(Number(resolved)))) {
        const num = Number(resolved);
        displayValue = Number.isFinite(num) ? num.toLocaleString("en-US", { maximumFractionDigits: 10 }) : String(resolved);
      } else {
      ...
      return (
        <div className="w-full h-full bg-card/80 border rounded-xl p-2.5 flex flex-col items-center justify-center text-center">
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-0.5">{props.label}</span>
          <span className={`font-bold ${trendColor} ${isAddress ? "text-sm font-mono" : "text-lg"}`} title={isAddress ? String(resolved) : undefined}>
            {displayValue}
            {props.unit && <span className="text-xs font-normal text-muted-foreground ml-1">{props.unit}</span>}
          </span>
        </div>
      );
    },
    ...
    /* ── Metric Row ───────────────────────────────────────── */
    MetricRow: ({ children }) => (
      <div className="w-full grid grid-cols-2 gap-2.5 sm:grid-cols-4" style={{ gridColumn: "1 / -1" }}>
        {children}
      </div>
    ),
    ...
    PriceChart: ({ props }) => {
      ...
      const pairAddress = typeof props.pair === "string" && props.pair !== "" ? props.pair : undefined;
      ...
      const changePct = firstTrade !== null && firstTrade.priceEth > 0
      ...
      const rangePct = lastTrade !== null && priceRange > 0
      ...
      /* ── Address truncation ──────────────────────────────── */
      const truncAddr = pairAddress != null && pairAddress.length > 14
      ...
      if (hasTrades) {
        for (let i = 0; i < trades.length; i++) {
          ...
          tradeDots.push(
            <ReferenceDot key={`td-${i}`} x={t.timestamp} y={t.priceEth}
              r={isLast ? 4 : 2.5}
              fill={t.side === "buy" ? "#22c55e" : "#ef4444"}
              fillOpacity={isLast ? 0.9 : 0.5} stroke={isLast ? (t.side === "buy" ? "#22c55e" : "#ef4444") : "none"}
              strokeWidth={isLast ? 1 : 0} strokeOpacity={0.3} />,
          );
        }
      }

      /* ── Tooltip ─────────────────────────────────────────── */
      const ChartTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: TradeData }> }) => {
        ...
        return (
          <div className="bg-popover/95 backdrop-blur-xl border border-border-light/50 rounded-xl px-3.5 py-2.5 shadow-[0_12px_48px_rgba(0,0,0,0.65)]">
            <div className="flex items-center gap-2 mb-1.5">
              <span className={`size-2 rounded-full ${d.side === "buy" ? "bg-emerald-400" : "bg-red-400"}`} />
            ...
            <div className="font-mono text-[15px] font-bold text-foreground leading-tight">
              {fmtEth(d.priceEth)} <span className="text-muted-foreground text-[9px] font-normal">ETH</span>
            </div>
            <div className="font-mono text-[11px] text-muted-foreground mt-0.5">{fmtUsd(d.priceUsd)}</div>
            <div className="border-t border-border/40 mt-2 pt-1.5 flex items-center gap-3 text-[9px] text-muted-foreground">
              ...
              <span className="font-mono">{d.trader.slice(0, 6)}…{d.trader.slice(-4)}</span>
            </div>
          </div>
        );
      };
      ...
      const emptyMsg = pairAddress === undefined
      ...
      return (
        <div className="w-full h-full bg-card/80 border rounded-xl flex flex-col overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 pt-3 pb-0 shrink-0">
            <div className="flex items-center gap-2">
              ...
              {truncAddr != null && (
                <span className="text-[10px] font-mono text-muted-foreground/50" title={pairAddress}>{truncAddr}</span>
              )}
            </div>
            <span className="text-[9px] font-bold px-1.5 py-[2px] rounded bg-emerald-500/10 text-emerald-400 uppercase tracking-wider">RobinPump</span>
          </div>
          ...
          {hasTrades && lastTrade !== null && (
            <div className="px-4 pt-2.5 pb-1 shrink-0">
              ...
              {/* USD price */}
              <div className="text-[11px] font-mono text-muted-foreground mb-3">{fmtUsd(lastTrade.priceUsd)}</div>
              {/* Stats row */}
              <div className="flex items-center gap-5 text-[10px] mb-2.5">
                <div>
                  <span className="text-muted-foreground/60 uppercase tracking-wider text-[9px]">High</span>
                  <span className="font-mono text-foreground ml-1.5">{fmtEth(highEth)}</span>
                </div>
                <div>
                  <span className="text-muted-foreground/60 uppercase tracking-wider text-[9px]">Low</span>
                  <span className="font-mono text-foreground ml-1.5">{fmtEth(lowEth)}</span>
                </div>
                <div>
                  <span className="text-muted-foreground/60 uppercase tracking-wider text-[9px]">Vol</span>
                  <span className="font-mono text-foreground ml-1.5">{totalVolEth.toFixed(4)} ETH</span>
                </div>
              </div>
              {/* Price range bar — shows where current price sits between high and low */}
              <div className="flex items-center gap-2">
                <span className="text-[8px] font-mono text-muted-foreground/40 w-8 text-right shrink-0">Low</span>
                <div className="flex-1 h-[3px] bg-border/20 rounded-full relative">
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 size-[7px] rounded-full bg-foreground shadow-[0_0_6px_rgba(255,255,255,0.3)] transition-all duration-500"
                    style={{ left: `calc(${rangePct}% - 3.5px)` }} />
                </div>
                <span className="text-[8px] font-mono text-muted-foreground/40 w-8 shrink-0">High</span>
              </div>
            </div>
          )}
          ...
          {isLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 px-4">
              ...
              <div className="w-full max-w-[240px] space-y-1.5 opacity-30">
                ...
                <div className="h-[2px] w-full bg-border rounded-full animate-pulse" style={{ animationDelay: "100ms" }} />
                <div className="h-[2px] w-2/3 bg-border rounded-full animate-pulse" style={{ animationDelay: "200ms" }} />
              </div>
            </div>
          ) : hasTrades ? (
            <div className="flex-1 min-h-0 px-1 pb-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trades} margin={{ top: 8, right: 8, bottom: 4, left: 8 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 6" stroke="var(--border)" strokeOpacity={0.2} vertical={false} />
                  <XAxis dataKey="timestamp" type="number" domain={["dataMin", "dataMax"]}
                    ...
                    axisLine={false} tickLine={false} minTickGap={60} dy={4} />
                  ...
                  <Area type="monotone" dataKey="priceEth" stroke={strokeColor} strokeWidth={2}
                    fill={`url(#${gradId})`} dot={false} filter={`url(#${glowId})`}
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-8">
              <div className="relative mb-4">
                <div className="size-12 rounded-full bg-emerald-500/6 border border-emerald-500/15 flex items-center justify-center">
                  <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-400">
              ...
              <span className="text-[11px] font-bold text-text-secondary uppercase tracking-wider mb-1">{emptyTitle}</span>
              <span className="text-[10px] text-muted-foreground text-center max-w-[260px] leading-relaxed">{emptyMsg}</span>
            </div>
          )}
    ...
    NumberInput: ({ props, emit }) => {
      ...
      return (
        <div className="w-full space-y-1.5">
          <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.08em]">{props.label}</Label>
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
          <div className="relative">
            <Input type="number" value={val} min={props.min} max={props.max} step={props.step ?? 1}
              className="font-mono"
              onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) { setState(path, v); emit?.("press"); } }} />
            {props.unit && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">{props.unit}</span>}
          </div>
        </div>
      );
    },
    ...
    EthInput: ({ props, emit }) => {
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
          <div className="relative">
            <Input type="number" value={val} min={props.min} max={props.max} step={step}
              className="font-mono"
              onChange={(e) => {
            ...
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">ETH</span>
          </div>
        </div>
      );
    },
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          <div className="relative">
            ...
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">USD</span>
          </div>
        </div>
      );
    },
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          <div className="relative">
            ...
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">bps</span>
          </div>
        </div>
      );
    },
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          <div className="relative">
            ...
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">%</span>
          </div>
        </div>
      );
    },
    ...
    TextInput: ({ props, emit }) => {
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
          <div className="relative">
            <Input type="text" value={val} placeholder={props.placeholder} className="font-mono"
              onChange={(e) => { setState(path, e.target.value); emit?.("press"); }} />
            {props.unit && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground font-medium">{props.unit}</span>}
          </div>
        </div>
      );
    ...
    SelectInput: ({ props, emit }) => {
      ...
      if (options.length === 0 && val !== undefined) options.push({ label: String(val), value: String(val) });
      return (
        <div className="w-full space-y-1.5">
          <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.08em]">{props.label}</Label>
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
      ...
      return (
        <div className="flex items-center justify-between gap-4 py-1">
          <div>
            ...
            {props.description && <span className="text-[11px] text-muted-foreground/70">{props.description}</span>}
          </div>
          <Switch checked={on} onCheckedChange={(checked) => { setState(path, checked); emit?.("press"); }} />
        </div>
      );
    },
    ...
    AddressInput: ({ props, emit }) => {
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
          <div className="relative">
            <Input type="text" value={val} placeholder={props.placeholder ?? "0x…"} className="font-mono"
              onChange={(e) => {
                setState(path, e.target.value.trim());
                emit?.("press");
              }} />
            <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-medium ${isValid ? "text-emerald-400/80" : "text-red-400/80"}`}>
              {isValid ? "ok" : "bad"}
            </span>
          </div>
        </div>
    ...
    IntervalInput: ({ props, emit }) => {
      ...
      return (
        <div className="w-full space-y-1.5">
          ...
          {props.description && <p className="text-[11px] text-muted-foreground/70">{props.description}</p>}
          <div className="relative">
            <Input type="text" value={val} placeholder="5m" className="font-mono"
              onChange={(e) => {
                setState(path, e.target.value.trim());
                emit?.("press");
              }} />
          ...
          <p className="text-[10px] text-muted-foreground/70 font-mono">Format: 30s · 5m · 1h · 1d · once</p>
        </div>
      );
    },
    ...
    ParameterGroup: ({ props, children }) => (
      <div className="flex-1 min-w-0 bg-card/60 border rounded-xl p-3">
        <h3 className="text-xs font-bold text-foreground uppercase tracking-[0.08em] mb-1">{props.title}</h3>
        {props.description && <p className="text-[11px] text-muted-foreground mb-2">{props.description}</p>}
    ...
    /* ── Heading ──────────────────────────────────────────── */
    Heading: ({ props }) => {
      const cls = props.level === "h1" ? "font-display text-[2rem] leading-tight text-foreground"
        : props.level === "h3" ? "text-sm font-semibold text-foreground"
    ...
    /* ── Text ─────────────────────────────────────────────── */
    Text: ({ props }) => {
      const cls = props.variant === "caption" ? "text-xs text-muted-foreground"
        : props.variant === "code" || props.variant === "mono" ? "font-mono text-[13px] text-text-secondary bg-card/80 rounded px-2 py-1"
    ...
    Badge: ({ props }) => {
      const BADGE: Record<string, string> = {
        green: "bg-emerald-500/10 text-emerald-400", amber: "bg-amber-500/10 text-amber-400",
        red: "bg-red-500/10 text-red-400", blue: "bg-blue-500/10 text-blue-400",
        violet: "bg-violet-500/10 text-violet-400", zinc: "bg-secondary text-muted-foreground",
      };
      return (
        <span className={`inline-flex text-[10px] font-bold px-2 py-[3px] rounded-md ${BADGE[props.color ?? "zinc"]}`}>
          {props.label}
        </span>
      );
    },
    ...
    InfoBox: ({ props }) => {
      ...
      return (
        <div className={`border rounded-xl p-4 ${V[props.variant ?? "info"]}`} style={{ gridColumn: "1 / -1" }}>
    ...
    TradeHistory: ({ props }) => {
      ...
      for (let i = trades.length - 1; i >= 0 && rows.length < maxItems; i--) {
        ...
        const truncHash = t.txHash.length > 14 ? `${t.txHash.slice(0, 6)}…${t.txHash.slice(-4)}` : t.txHash;
        const pairLabel = t.pairAddress.length > 14
        ...
        rows.push(
          <div key={`${t.idx}:${t.txHash}`} className="flex items-center gap-2 py-2 px-1">
            <span className={`size-1.5 rounded-full shrink-0 ${isBuy ? "bg-emerald-400" : "bg-red-400"}`} />
            <span className={`text-[10px] font-bold uppercase tracking-wider w-8 shrink-0 ${isBuy ? "text-emerald-400" : "text-red-400"}`}>{t.side}</span>
            <span className="text-sm font-mono text-foreground shrink-0">{amountLabel}</span>
            <span className="text-[11px] text-muted-foreground truncate min-w-0" title={t.pairAddress}>{pairLabel}</span>
            <span className="ml-auto text-[10px] text-muted-foreground font-mono shrink-0">{timeLabel}</span>
            <a
      ...
      const isInitialLoad = strategyId !== undefined && perfQuery.data === undefined && perfQuery.isFetching;
      if (strategyId === undefined) {
        ...
        body = (
          <div className="py-3 px-1 text-[11px] text-red-300">
      ...
      return (
        <div className="w-full bg-card/80 border border-border rounded-xl p-3 overflow-hidden" style={{ gridColumn: "1 / -1" }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Recent Trades</span>
    ...
    TradeRow: ({ props }) => {
      ...
      const truncHash = props.hash != null && props.hash.length > 14
        ? `${props.hash.slice(0, 6)}…${props.hash.slice(-4)}`
        : props.hash;
      return (
        <div className="flex items-center gap-2 py-2 px-1">
          <span className={`size-1.5 rounded-full shrink-0 ${isBuy ? "bg-emerald-400" : "bg-red-400"}`} />
          <span className={`text-[10px] font-bold uppercase tracking-wider w-8 shrink-0 ${isBuy ? "text-emerald-400" : "text-red-400"}`}>{props.side}</span>
          ...
          <span className="ml-auto text-[10px] text-muted-foreground font-mono shrink-0">{props.time}</span>
          {truncHash != null && (
            <a href={`https://basescan.org/tx/${props.hash}`} target="_blank" rel="noopener noreferrer"
              className="text-[10px] font-mono text-gold-dim hover:text-gold-bright transition-colors shrink-0" title={props.hash}>
              {truncHash}
            </a>
          )}
        </div>
      ...
      return (
        <div className="w-full">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{props.label}</span>
            <span className={`text-xs font-mono font-bold ${c.text}`}>{clamped}{props.unit ?? "%"}</span>
          </div>
          <div className={`w-full h-1.5 rounded-full ${c.bg}`}>
            <div className={`h-full rounded-full ${c.bar} transition-all duration-500 ease-out`} style={{ width: `${clamped}%` }} />
          </div>
        </div>
      );
    },
    ...
    StatusIndicator: ({ props }) => {
      ...
      return (
        <div className="flex items-center gap-2.5 bg-card/80 border border-border rounded-xl p-3" style={{ gridColumn: "1 / -1" }}>
          <span className="relative flex size-2.5 shrink-0">
            {s.pulse && <span className={`animate-ping absolute inline-flex size-full rounded-full ${s.dot} opacity-50`} />}
          ...
          <div className="min-w-0">
            ...
            {props.description != null && <p className="text-[11px] text-muted-foreground mt-0.5">{props.description}</p>}
          </div>
        </div>
      );
    },
    ...
    CoinCard: ({ props }) => {
      ...
      const truncAddr = props.pairAddress != null && props.pairAddress.length > 14
        ...
        <div className="flex items-center gap-3 bg-card/80 border border-border rounded-xl p-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold text-muted-foreground uppercase">{props.symbol}</span>
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              {props.priceEth != null && (
                <span className="text-xs font-mono text-foreground">{props.priceEth} <span className="text-muted-foreground text-[10px]">ETH</span></span>
              )}
              {props.priceUsd != null && (
                <span className="text-[10px] font-mono text-muted-foreground">${props.priceUsd}</span>
              )}
            </div>
            {truncAddr != null && (
              <span className="text-[10px] font-mono text-muted-foreground/60 mt-0.5 block" title={props.pairAddress}>{truncAddr}</span>
            )}
          </div>
        </div>
      );
    ...
    CoinList: ({ props, children }) => (
      <div className="w-full" style={{ gridColumn: "1 / -1" }}>
        {props.title != null && (
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-2">{props.title}</span>
        )}
    ...
    AlertBanner: ({ props }) => {
      ...
      const VARIANTS: Record<string, { bg: string; border: string; icon: string; text: string }> = {
        info: { bg: "bg-blue-500/[0.06]", border: "border-blue-500/20", icon: "text-blue-400", text: "text-blue-200" },
        warning: { bg: "bg-amber-500/[0.06]", border: "border-amber-500/20", icon: "text-amber-400", text: "text-amber-200" },
        success: { bg: "bg-emerald-500/[0.06]", border: "border-emerald-500/20", icon: "text-emerald-400", text: "text-emerald-200" },
        error: { bg: "bg-red-500/[0.06]", border: "border-red-500/20", icon: "text-red-400", text: "text-red-200" },
      };
      const ICONS: Record<string, ReactNode> = {
        info: <svg width="16" height="16" fill="none" viewBox="0 0 16 16"><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.5" /><path d="M8 7v4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /><circle cx="8" cy="5" r="0.75" fill="currentColor" /></svg>,
        warning: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 16 16"><path d="M8 1.5L1 14h14L8 1.5z" /><path d="M8 6v3.5" /><circle cx="8" cy="11.5" r="0.5" fill="currentColor" stroke="none" /></svg>,
        success: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 16 16"><circle cx="8" cy="8" r="6" /><path d="M5.5 8l2 2 3-3.5" /></svg>,
        error: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 16 16"><circle cx="8" cy="8" r="6" /><path d="M6 6l4 4M10 6l-4 4" /></svg>,
      };
      const v = VARIANTS[props.variant] ?? VARIANTS.info!;
      return (
        <div className={`${v.bg} border ${v.border} rounded-xl p-3 flex items-start gap-2.5`} style={{ gridColumn: "1 / -1" }}>
    ...
    ScheduleDisplay: ({ props }) => (
      <div className="w-full bg-card/80 border border-border rounded-xl p-3" style={{ gridColumn: "1 / -1" }}>
        <div className="flex items-center gap-2 mb-2.5">
          <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="text-gold-dim" viewBox="0 0 16 16">
            <circle cx="8" cy="8" r="6" /><path d="M8 4.5v3.5l2.5 2" />
          </svg>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-4">
          ...
          {props.runCount != null && (
            <div>
              ...
              <span className="text-sm font-mono font-bold text-foreground">{props.runCount.toLocaleString()}</span>
            </div>
          )}
        </div>
      </div>
    ...
    Stat: ({ props }) => {
      const ICONS: Record<string, ReactNode> = {
        wallet: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 20 20"><rect x="2" y="5" width="16" height="12" rx="2" /><path d="M2 9h16" /><circle cx="14.5" cy="12.5" r="1" fill="currentColor" stroke="none" /></svg>,
        chart: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 20 20"><path d="M2 15l5-8 4 5 7-9" /></svg>,
        bolt: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 20 20"><path d="M11 2L4 11h5l-1 7 7-9h-5l1-7z" /></svg>,
        clock: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" /><path d="M10 5v5l3.5 3.5" /></svg>,
        coins: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" viewBox="0 0 20 20"><circle cx="7.5" cy="10.5" r="5.5" /><path d="M13 5a5.5 5.5 0 010 11" /></svg>,
        trending: <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 20 20"><path d="M3 15l4-4 3 3 7-8" /><path d="M14 6h3v3" /></svg>,
      };
      ...
      return (
        <div className="flex-1 min-w-0 bg-card/80 border border-border rounded-xl p-3">
          ...
          {props.description != null && <p className="text-[11px] text-muted-foreground mt-1">{props.description}</p>}
        </div>
      );
    },
