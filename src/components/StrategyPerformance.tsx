...
export function StrategyPerformance({ strategyId, isActive = false }: {
  strategyId: string;
  isActive?: boolean;
}) {
  ...
  if (perfQuery.isError) {
    loadError = perfQuery.error instanceof Error ? perfQuery.error.message : "Failed to load performance";
  }
          ...
          <div className="flex items-end gap-3">
            <span
              className={`font-display text-[2rem] sm:text-[2.6rem] leading-none font-bold tracking-tight tabular-nums ${
                isPositive ? "text-emerald-400 animate-pnlGlowGreen" : "text-red-400 animate-pnlGlowRed"
              }`}
            >
              {fmtPnl(animatedPnl)}
            </span>
            <span className="text-muted-foreground text-sm font-mono mb-1">ETH</span>
            <span className={`text-sm font-bold font-mono mb-1 ${isPositive ? "text-emerald-400" : "text-red-400"}`}>
...
function EquityCurveChart({ data, strategyId }: { data: EquityPoint[]; strategyId: string }) {
  ...
  return (
    <div className="w-full rounded-xl bg-card/60 border border-border/50 overflow-hidden animate-fadeSlideUp"
      ...
      <div className="h-[180px] sm:h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 12, right: 12, bottom: 4, left: 12 }}>
            ...
            <CartesianGrid
              strokeDasharray="3 6" stroke="var(--border)" strokeOpacity={0.25} vertical={false}
            />
            <XAxis
            ...
            />
            <YAxis
              dataKey="pnlEth" domain={["auto", "auto"]}
              tickFormatter={(v: number) => fmtPnl(v)}
              tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "var(--font-mono)" }}
              axisLine={false} tickLine={false} width={70} orientation="right"
            />
            <ReferenceLine y={0} stroke="var(--gold-dim)" strokeDasharray="4 4" strokeOpacity={0.5} />
            <Tooltip
...
function EquityTooltip({ active, payload }: {
  ...
  return (
    <div className="bg-popover/95 backdrop-blur-xl border border-border-light/50 rounded-xl px-3.5 py-2.5 shadow-[0_12px_48px_rgba(0,0,0,0.65)]">
      <span className="text-[9px] text-muted-foreground block mb-1">{fmtTimeFull(d.timestamp)}</span>
      ...
      <div className="bg-card/60 border border-border/50 rounded-xl p-3">
        ...
        <div className="flex justify-between mt-1">
          ...
          <span className="text-[8px] font-mono text-red-400/60">
            {summary.totalTrades > 0 ? summary.totalTrades - Math.round(winPct * summary.totalTrades / 100) : 0}L
          </span>
        </div>
      </div>
      ...
      <div className="bg-card/60 border border-border/50 rounded-xl p-3">
        ...
        <div className="flex items-center gap-1.5 mt-2">
          <span className="text-[9px] font-mono text-muted-foreground">
            Avg: <span className={summary.avgTradePnlEth >= 0 ? "text-emerald-400" : "text-red-400"}>
      ...
      <div className="h-[200px] sm:h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={trades} margin={{ top: 12, right: 12, bottom: 4, left: 12 }}>
            <CartesianGrid
              strokeDasharray="3 6" stroke="var(--border)" strokeOpacity={0.2} vertical={false}
            />
            <XAxis
            ...
            <YAxis
              ...
              tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "var(--font-mono)" }}
              axisLine={false} tickLine={false} width={50} orientation="right"
              label={{
                value: "mETH",
                position: "top",
                offset: -2,
                style: { fill: "var(--muted-foreground)", fontSize: 8, fontFamily: "var(--font-mono)" },
              }}
            />
            <ReferenceLine
              y={0} stroke="var(--gold-dim)" strokeDasharray="6 3" strokeWidth={1.5} strokeOpacity={0.6}
            />
            <Tooltip
            ...
            />
            <Bar
              dataKey="pnlEth" radius={[3, 3, 0, 0]} barSize={barSize}
  ...
  return (
    <div className="bg-popover/95 backdrop-blur-xl border border-border-light/50 rounded-xl px-3.5 py-2.5 shadow-[0_12px_48px_rgba(0,0,0,0.65)] min-w-[180px]">
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className={`text-[10px] font-bold uppercase tracking-wider ${d.side === "buy" ? "text-emerald-400" : "text-red-400"}`}>
          {d.side}
        </span>
      ...
      <div className="flex items-baseline gap-2">
        <span className={`font-mono text-[15px] font-bold leading-tight ${isProfit ? "text-emerald-400" : "text-red-400"}`}>
          {fmtPnl(d.pnlEth)} ETH
        </span>
      ...
      <div className="mt-1.5 text-[9px] text-muted-foreground/60">
        Cumulative: <span className={`font-mono font-bold ${d.cumulativePnlEth >= 0 ? "text-emerald-400/60" : "text-red-400/60"}`}>
